...
export function FilterBar({ cases, filters, onChange, showEventFilter, showSearch }: Props) {
  return (
    <section
      aria-label="Report filters"
      className="mb-6 grid gap-4 border border-line bg-card p-4"
    >
      <div>
        <p className="text-xs font-medium tracking-wide text-ink-soft uppercase">Date range</p>
      ...
      <fieldset>
        <legend className="text-xs font-medium tracking-wide text-ink-soft uppercase">Cases</legend>
        <div className="mt-2 flex flex-wrap gap-2">
          <button
            type="button"
            className={[
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <fieldset>
          <legend className="text-xs font-medium tracking-wide text-ink-soft uppercase">Access</legend>
          <div className="mt-2 flex flex-wrap gap-2">
            {(["github_direct", "wix_embedded", "unknown"] as DeliveryContext[]).map((t) => (
              <label key={t} className="flex items-center gap-2 border border-line bg-paper px-2 py-1.5 text-sm">
                ...
                />
                {t === "github_direct" ? "GitHub direct" : t === "wix_embedded" ? "Wix embed" : "Unknown"}
              </label>
            ))}
          </div>
        </fieldset>
        <fieldset>
          <legend className="text-xs font-medium tracking-wide text-ink-soft uppercase">Device</legend>
          <div className="mt-2 flex flex-wrap gap-2">
      ...
      {showSearch ? (
        <label className="block text-sm">
          <span className="text-xs font-medium tracking-wide text-ink-soft uppercase">Search</span>
          <input
            type="search"
            value={filters.search}
            onChange={(e) => onChange({ ...filters, search: e.target.value })}
            placeholder="Case name, key, or anonymous session ID"
